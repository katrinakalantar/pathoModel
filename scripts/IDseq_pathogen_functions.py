...
def calculate_scores(df_rna, df_dna, rna, dna, output_file, output_directory, subtract_dna = None, subtract_rna = None):
    ...
    f.write("\nRNA\n")
    f.close()
    t1 = df_rna[df_rna['genus_NT_rpm RNA'] < 1]
    t2 = t1[t1['name'].isin(list(combined.head(n=20)['Species_Assignment']))]
    t2[['category_name','genus_taxid','name','NT_rpm RNA','NR_rpm RNA']].head(n=20).to_csv("./output/" + output_directory + "/TableS4_lowcount.csv", sep='\t', mode='a',index=False)
    f = open('./output/' + output_directory + '/TableS4_lowcount.csv','a')
    f.write("\nDNA\n")
    f.close()
    t1 = df_dna[df_dna['genus_NT_rpm DNA'] < 1]
    t2 = t1[t1['name'].isin(list(combined.head(n=20)['Species_Assignment']))]
    t2[['category_name','genus_taxid','name','NT_rpm DNA','NR_rpm DNA']].head(n=20).to_csv("./output/" + output_directory + "/TableS4_lowcount.csv", sep='\t', mode='a',index=False)
    #####
...
def main(BMs, dna_dir, rna_dir, file_pairs_dict, output_directory, metadata, subtract_dna = None, subtract_rna = None, investigative=False):
    ...
    for bm in list_of_BMs:
        ...
        dna_csv_files = ['./' + bm + '/' + dna_dir + '/' + list(file_pairs_dict.keys())[i].lower() +'.csv' for i in range(len(file_pairs_dict))]
        rna_csv_files = ['./' + bm + '/' + rna_dir + '/' + list(file_pairs_dict.values())[i].lower() +'.csv' for i in range(len(file_pairs_dict))]
        ...
        for c in range(len(rna_csv_files)):  #iterate through all csv files in the directory
            ...
            #print(df_dna.sum(axis=0)['NT_rpm'])
            #print(df_rna.sum(axis=0)['NT_rpm'])

            '''

            f = open('./output/' + output_directory + '/TableS4.csv','a')   # write the RNA filename to the output file
            f.write("\n\n")
            f.write('TA-'+str(metadata.loc[metadata['sample_name_RNA'] == rna.split('/')[-1].split('.')[0]].index[0]) + "\n")
            f.close()
